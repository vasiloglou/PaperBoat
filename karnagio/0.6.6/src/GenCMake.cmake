...
## Add a plugin defined by an entry in src/plugin
#
# Plugins are distinguished with prefix 'plugin' and are written
# to build subdirectory plugin.  On Linux systems, shared libraries reuse
# the project version extracted from the project root directory.
function(GenCMake_add_plugin PLUGIN_PATH)
  GenCMake_configure(${PLUGIN_PATH} PLUGIN_NAME SOURCES)
 
  verbose(STATUS "GenCMake:   Found library 'plugin${PLUGIN_NAME}'")
  add_library(plugin_${PLUGIN_NAME} MODULE ${SOURCES})
  set_target_properties(plugin_${PLUGIN_NAME} PROPERTIES
    LIBRARY_OUTPUT_DIRECTORY ${GenCMake_PLUGIN_OUT_DIR}
    ARCHIVE_OUTPUT_DIRECTORY ${GenCMake_PLUGIN_OUT_DIR}
    RUNTIME_OUTPUT_DIRECTORY ${GenCMake_PLUGIN_OUT_DIR}
    OUTPUT_NAME plugin_${PLUGIN_NAME} VERSION ${GenCMake_VERSION})

  target_link_libraries(plugin_${PLUGIN_NAME} ${GenCMake_LIBRARIES})
  message(STATUS ${GenCMake_LIBRARIES})
...
function(GenCMake_add TARGET_TYPE TARGET_DIR)
  ...
  foreach(TARGET ${TARGETS})
    ...
    elseif(TARGET_TYPE STREQUAL "PLUGINS")
      GenCMake_add_plugin(${TARGET})
...
default(GenCMake_PLUGIN_SRC_DIR plugin CACHE PATH
  "Source directory for automatically configured plugins.")
...
default(GenCMake_PLUGIN_OUT_DIR plugin CACHE PATH
...
if(GenCMake_DECLARE_PROJECT)
  ...
  if(CMAKE_COMPILER_IS_GNUCXX)
    append(CMAKE_CXX_FLAGS " -Wall  -Wno-unused-local-typedefs -Wno-sign-compare -fno-strict-aliasing -pthread -std=c++0x ")
...
if(GenCMake_DECLARE_TARGETS)
  ...
  GenCMake_add(PLUGINS ${GenCMake_PLUGIN_SRC_DIR})
